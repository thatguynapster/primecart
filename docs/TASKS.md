...
| #    | Task                                                                          | Status | Done |
| ---- | ----------------------------------------------------------------------------- | ------ | ---- |
| 10.1 | Order list dashboard with status filters (incl. `EXPIRED`)                    | [x] added a "Needs review" saved view (`status: EXPIRED, paymentStatus: PAID`) alongside the existing views; the plain `EXPIRED`/unpaid case was already visible via "All orders" | 2026-08-16 |
| 10.2 | Order detail view (line item snapshots, shipping address)                     | [x] `/dashboard/orders/[orderId]` — line items, customer, delivery address, notes, channel | 2026-08-16 |
| 10.3 | Manual order creation for walk-in / WhatsApp sales — `source: MANUAL`         | [x] `/dashboard/orders/new` — product/variant picker, re-derives price/stock from the live catalogue, reuses `reserveStock` + `findOrCreateCustomer`. A "Channel" field also allows `WHATSAPP` for that source | 2026-08-16 |
| 10.4 | Status transitions PENDING → CONFIRMED → PROCESSING → SHIPPED → DELIVERED → CANCELLED | [x] `src/lib/orders/transitions.ts` defines the legal chain + cancellable-from set; `advanceOrderStatus`/`cancelOrder` in `orders/actions.ts` enforce it server-side from the order's own current status, never a client-supplied value | 2026-08-16 |
| 10.5 | Mark as paid / mark as fulfilled actions                                      | [x] `markOrderPaid` records a payment confirmed outside Paystack (cash/mobile money); "mark as fulfilled" is `advanceOrderStatus` walking the chain above | 2026-08-16 |
| 10.6 | `orderNumber` generation                                                      | [x] built in Phase 9 — `generateOrderNumber()` | 2026-08-16 |
| 10.7 | `EXPIRED` is system-set only — not a status a merchant can pick manually      | [x] `nextStatus()` never returns `EXPIRED`; no UI control offers it. Verified live that the chain and cancel set never produce it | 2026-08-16 |
| 10.8 | **Surface `status: EXPIRED` + `paymentStatus: PAID` distinctly** — a late-recovered payment that could not be fulfilled. No visibility exists yet outside the database; see Phase 9's post-review fix note | [x] "Needs review" saved view, a distinct accent badge in the order list ("Needs review" instead of "Expired"), a review banner on the order detail page explaining what happened, and rolled into the side-rail's orders-needing-action count | 2026-08-16 |

## Phase 11 — Customer Records

| #    | Task                                                       | Status | Done |
| ---- | ------------------------------------------------------------ | ------ | ---- |
| 11.1 | Auto-create/match Customer from storefront orders          | [x] `findOrCreateCustomer()`, built in Phase 9 for storefront checkout | 2026-08-16 |
| 11.2 | Auto-create/match Customer from manual orders              | [x] Phase 10's manual order form calls the same `findOrCreateCustomer()` | 2026-08-16 |
| 11.3 | Customer list (name, email, phone)                         | [x] built in Phase 8b (`/dashboard/customers`, `listCustomers()` aggregation) — name, contact (email or phone), order count, spend, last order date. Rows now link to the detail page below | 2026-08-16 |
| 11.4 | Customer detail with purchase history                      | [x] `/dashboard/customers/[customerId]` — contact info, orders/spent/last-order stat tiles, full order history table linking into `/dashboard/orders/[orderId]` | 2026-08-16 |
| 11.5 | Confirm no standalone customer creation flow exists        | [x] verified live — `prisma.customer.create` has exactly one call site in the whole codebase, `findOrCreateCustomer()`. No route, action, or form creates a `Customer` any other way | 2026-08-16 |
...
| #    | Task                                                   | Status | Done |
| ---- | -------------------------------------------------------- | ------ | ---- |
| 12.1 | Total sales aggregation: daily / weekly / monthly      | [x] `getSalesReport(merchantId, granularity)` — daily/weekly share one trailing-day bucketing helper (`dailyPaidTotals`), monthly reuses the existing true-calendar-month `getMonthlyRevenue`. `getSalesSeries` (Overview's 14-day widget) refactored to share the same helper, behaviour unchanged | 2026-08-16 |
| 12.2 | Best selling products aggregation                      | [x] `getBestSellers()`, built in Phase 8b for the Overview page — now also surfaced on Analytics (limit 8) | 2026-08-16 |
| 12.3 | Current stock value aggregation                        | [x] `getOverviewKpis().stockValue` (a total) and `getStockValueByCategory()` (by category), both built in Phase 8b — the total is now also on the Analytics page, which previously only showed the by-category breakdown | 2026-08-16 |
| 12.4 | Dashboard UI for the three reports (mobile-first)      | [x] `/dashboard/analytics` — a daily/weekly/monthly toggle (`sales-report.tsx`, client-side only for the tab switch; all three series fetched server-side up front, no re-fetch on toggle), best sellers, stock value by category, all pre-existing from Phase 8b (revenue/orders/repeat-rate/refunds KPIs, channel split) | 2026-08-16 |
...
| #     | Task                                                                                                             | Status | Done |
| ----- | ------------------------------------------------------------------------------------------------------------------ | ------ | ---- |
| 13.1  | Plan code in env (`PAYSTACK_PLAN_CODE`) so test → live is a config change, not a code change                      | [x] already set from Phase 1; confirmed no code path hardcodes it — `src/app/billing/actions.ts` reads `process.env.PAYSTACK_PLAN_CODE` | 2026-08-16 |
| 13.2  | Daily cron endpoint — separate from the order-expiry cron; finds merchants where `trialExpiresAt < now` AND `subscriptionStatus: TRIAL` | [x] `/api/cron/expire-trials` → `expireLapsedTrials()` in `src/lib/subscription.ts`, claim-then-act race-safe like `expireAbandonedOrders` | 2026-08-16 |
| 13.3  | Protect the daily cron endpoint with `CRON_SECRET` the same way as 9.13                                          | [x] reuses `checkCronAuth` unchanged | 2026-08-16 |
| 13.4  | *(Owner)* Register the daily job on cron-job.org                                                                  | [ ] instructions already written in `docs/VERCEL_SETUP.md` §8 ("The second job (Phase 13)") — endpoint now exists to test against | |
| 13.5  | Payment page collecting card details, initiating a Paystack subscription against the plan code                   | [x] `/billing` — card entry itself happens on Paystack's own hosted page (`authorization_url`), same pattern as the storefront checkout | 2026-08-16 |
| 13.5a | **Resolve D-14 first** — does `/billing` stay sign-in-only, or split into a public explainer + protected payment page? | [x] resolved by owner: stays sign-in-only, no split | 2026-08-16 |
| 13.6  | On successful subscription: store `paystackSubscriptionCode`, set `subscriptionStatus: ACTIVE`                    | [x] `activateSubscription()`, called from the `subscription.create` webhook (13.8) — also reactivates the storefront if it had been switched off | 2026-08-16 |
| 13.7  | Unpaid after trial expiry → `subscriptionStatus: EXPIRED` + `storefront.isActive: false`                          | [x] `expireLapsedTrials()` — see 13.2 | 2026-08-16 |
| 13.8  | Webhook `subscription.create` → confirm active, update merchant record                                            | [x] matched by the merchant's account email (`@unique`), since Paystack's subscription object carries no PrimeCart id | 2026-08-16 |
| 13.9  | Webhook `invoice.payment_failed` → `subscriptionStatus: EXPIRED`, deactivate storefront                           | [x] matched by `paystackSubscriptionCode` (nested under `data.subscription` for this event), email fallback | 2026-08-16 |
| 13.10 | Webhook `subscription.disable` → same as `invoice.payment_failed`                                                 | [x] same `expireSubscription()`, matched by `paystackSubscriptionCode` (top-level for this event) | 2026-08-16 |
| 13.11 | Dashboard lockout for EXPIRED merchants (built in 3.7) verified end to end                                        | [~] verified the mechanism the proxy relies on (`getSubscriptionStatus`/`getMerchantBySubdomain` correctly reflect EXPIRED after a real state change, live); a real signed-in browser session hitting the redirect could not be scripted the same way task 6.12/8.10 needed a human — **owner to confirm once, same pattern** | 2026-08-16 |
| 13.12 | Merchant-facing subscription/trial status view                                                                    | [x] new Billing card on `/dashboard/settings` (status + trial days left, visible in every subscription state) alongside the pre-existing sidebar trial card (TRIAL only) | 2026-08-16 |
...
### Resolved 2026-08-16 by project owner

| ID   | Decision |
| ---- | -------- |
| D-14 | `/billing` stays sign-in-only. No split into a public explainer + protected payment page. |
...
| 2026-08-16 | **Phase 10 complete** — order detail view, manual order creation (`/dashboard/orders/new`, reusing `reserveStock`/`findOrCreateCustomer`), merchant-driven status transitions and mark-as-paid (`src/lib/orders/transitions.ts` keeps `EXPIRED` provably unreachable through any merchant action), cancel-with-stock-restoration. 10.8's late-recovered-payment visibility gap closed with a "Needs review" saved view, a distinct list badge, a detail-page banner, and inclusion in the side-rail's action count. Verified live against the real `gadgethub` catalogue: 21/21 checks (status chain, cancel/restore, mark-paid, stock fully returned to its original count afterward), fixtures cleaned up. Also closes 11.1/11.2, pulled forward — both storefront and manual orders now share one customer-matching path |
| 2026-08-16 | **Platform notifications (Resend) built** — not part of the original handover; scoped in `docs/NOTIFICATIONS.md` with the owner before building, same pattern as `STOREFRONT_BRANDING.md`/D-15. Three merchant-only events: new paid order, needs-review (a late/oversold payment — closes the remaining visibility gap from task 10.8's fix), and low stock (fires once per crossing from a sale, never from a manual recount). `lowStockAlertedAt` added to `ProductVariant` as the dedupe marker. Verified live with real Resend sends (no mocking): 10/10 low-stock dedupe checks, 6/6 webhook notification checks, fixtures cleaned up. Found and fixed a real bug along the way — a raw `$set` of a JS `Date` through `$runCommandRaw` serialises as a string, not a BSON date, and broke the next Prisma Client read; fixed with MongoDB extended JSON, matching `dashboard/queries.ts`'s existing convention. Resend sending-domain verification is still an owner task before this reaches merchants other than the account's own signup email |
| 2026-08-16 | **Two owner-reported bugs in the new order list fixed.** (1) The order number in `/dashboard/orders` was a bare, unstyled `Link` covering only that cell — clicking elsewhere in the row did nothing and there was no visual cue it was clickable. Fixed with a stretched-link pattern (`after:absolute after:inset-0` on the link, `relative` on the row) plus accent colour/underline so the whole row navigates. (2) Manual orders without a delivery address showed "Guest" in the list despite having a real `Customer` record — `listOrders` only read `shippingAddress.name`, which manual orders can leave unset. Added a `$lookup` on `Customer` as a fallback so the list matches what the detail page already showed. Verified live: 2/2 checks (no-shippingAddress order now shows the customer's name; shippingAddress-bearing orders unaffected) |
| 2026-08-16 | **Phase 11 complete** — 11.1/11.2 were already closed (pulled forward into Phase 9/10's `findOrCreateCustomer()`), and the Phase 8b customer list turned out to already satisfy 11.3. What was missing: `/dashboard/customers/[customerId]` (11.4) — contact info, orders/spent/last-order stat tiles, full order history linking into the order detail page — plus the same stretched-link fix applied to the customer list's rows so they actually navigate there. 11.5 verified live rather than by inspection: exactly one `prisma.customer.create` call site in the entire codebase. 8/8 live checks (customer/order scoping by merchantId, order counts, the single-call-site grep), fixtures untouched since this was all reads |
| 2026-08-16 | **Owner-reported bug: customer list and customer detail disagreed on order count/last-order date.** Root cause: `listCustomers`'s aggregation pre-filtered its entire `$lookup` on `paymentStatus: "PAID"`, so a customer whose orders were all unpaid/pending showed 0 orders and no last-order date there, while the detail page (11.4, built without noticing this) correctly counted every order regardless of payment status. Fixed by only paid-filtering the `spent` sum — `orders` count and `lastOrder` now cover every order, matching the detail page. Also switched every "Placed"/"Last order" timestamp across the dashboard (orders list, order detail, customer list, customer detail, the overview's live feed) to a new shared `formatRelativeDate()` in `src/lib/format.ts` — "Just now", "5 minutes ago", "Yesterday", "Last week", "3 weeks ago", falling back to an absolute date past ~5 years — each with a `title` attribute carrying the exact timestamp on hover. Also removed a cruder duplicate `relativeTime()` that already existed on the dashboard overview page, now consolidated into the same helper. Verified live: 17/17 checks reproducing the exact reported scenario (a customer with only unpaid orders) plus the relative-date formatter's boundaries, fixtures cleaned up |
| 2026-08-16 | **Phase 12 complete** — most of the reporting work was already built ahead of schedule in Phase 8b (best sellers, stock value by category, revenue/orders/repeat-rate/refunds KPIs, channel split); what 12.1/12.4 were actually missing was the handoff's specific "daily / weekly / monthly view" for total sales, which only existed as two separate fixed charts (a 14-day daily widget on Overview, a 12-month chart on Analytics) with no weekly granularity and no way to switch between them. Added `getSalesReport(merchantId, granularity)`, refactored the existing daily aggregation into a shared `dailyPaidTotals()` helper reused by both the new function and the untouched `getSalesSeries()`, and built a client-side daily/weekly/monthly toggle (`sales-report.tsx`) — all three series fetched server-side up front, so switching tabs never re-hits the database. Also added best sellers and a total current-stock-value figure to the Analytics page itself, which previously only had the by-category breakdown. Verified live: 10/10 checks (bucket counts, a real order landing in the correct daily/weekly/monthly bucket simultaneously with no double-counting, the new function matching the untouched one bucket-for-bucket), fixtures cleaned up |
| 2026-08-16 | **D-14 resolved by owner**: `/billing` stays sign-in-only, no split into a public explainer. **Phase 13 built** — subscription payment via Paystack's plan-linked checkout (`/billing`, hosted card entry on Paystack's own page, same pattern as the storefront checkout), a daily trial-expiry cron (`/api/cron/expire-trials` → `expireLapsedTrials()`, claim-then-act race-safe like the order-expiry cron), and three new Paystack webhook events (`subscription.create` activates + stores the subscription code, `invoice.payment_failed`/`subscription.disable` both expire and deactivate the storefront — matched by `paystackSubscriptionCode` first, the merchant's `@unique` email as fallback since Paystack's subscription objects carry no PrimeCart id). A "Billing" status card added to `/dashboard/settings` (13.12), visible in every subscription state, alongside the pre-existing sidebar trial card. **Found and fixed a real Paystack API bug while verifying live**: `/transaction/initialize` rejects a `plan`-linked charge with no `amount`, contrary to what the docs imply about deriving it from the plan — fixed by fetching the plan's own amount first (`getPlan()`) and passing it explicitly, which also means the charged amount can never drift from what the plan is actually configured for. Verified live: 30/30 checks against a disposable fixture merchant (trial expiry, direct activate/expire, all three webhook events including the nested-vs-top-level subscription-code shape difference between `invoice.payment_failed` and `subscription.disable`, a plan-linked `charge.success` correctly *not* touching order-confirmation logic, a real Paystack test-mode call returning a working checkout URL, the cron endpoint's auth guard over live HTTP), fixture merchant deleted afterward. **13.11's dashboard-lockout verification is partial** — proved live that the proxy's own lookup functions correctly reflect EXPIRED immediately after a state change, but a real signed-in browser hitting the redirect needs the owner, same as tasks 6.12/8.10/9.20. 13.4 (register the cron job) and 13.13 (swap in the live plan code at deploy) remain owner tasks, both already documented in `docs/VERCEL_SETUP.md` §8 |
| 2026-08-16 | **Pre-deploy smoke test, run against the real `dev.primecart.app` deployment (not localhost) at the owner's request** — signed-in browser walkthrough of every dashboard page (Overview, Orders + detail, Products, Customers + detail, Analytics, Settings), a real storefront purchase through Paystack's test checkout (GH₵7,299.99, on-screen "Success" test card), and a real subscription payment through `/billing` (GH₵79, confirming the plan-amount fix charges the right figure live). No console errors on any page; every dashboard page's data matched what the underlying real orders should produce. **Found a real, blocking pre-deploy gap**: nothing in this project registers a webhook URL with Paystack — that is a manual one-time Paystack Dashboard setting, and it was never set for `dev.primecart.app`. Both the real order and the real subscription payment above succeeded on Paystack's side but sat unconfirmed in the dashboard indefinitely, because the payment confirmation never reached `/api/webhooks/paystack` at all. Confirmed the webhook *code* itself was not the problem by manually replaying the exact same signed payload Paystack would have sent — both confirmed within seconds. Also surfaced, incidentally: a real subscription (`SUB_anrlai3afdmeszh`) had been sitting paid-but-unconfirmed since roughly 40 minutes before this test began — evidently from an earlier, separate attempt — and is now correctly ACTIVE after the replayed webhook. **New §9 added to `docs/VERCEL_SETUP.md`** with the exact Dashboard steps (Settings → API Keys & Webhooks, one URL per Test/Live mode) and a verification recipe — this must be set for both `dev.primecart.app` and `primecart.app` before either environment's payments can ever confirm on their own. Not caught earlier because every previous verification pass (Phases 9, 10, 13) tested the webhook *route* directly or against a local server, which was the right call for proving the route's own logic but could not have caught a Dashboard setting that only a real deployment receiving real Paystack traffic would expose |
| 2026-08-16 | **A second, more serious bug found immediately after registering the webhook URL above** — with delivery now genuinely reaching the endpoint, real Paystack `charge.success` events verified their signature and logged correctly, but the matching order still never confirmed, silently, on every single delivery. Root cause, found by temporarily logging the full raw event body: `data.plan` is an **object** on every `charge.success` payload Paystack sends — `{}` for an ordinary storefront charge, `{plan_code: "..."}` only for a subscription-funding charge — never absent and never the bare string the route's type declared. The guard meant to skip subscription charges, `!event.data.plan`, was therefore **always false** (`{}` is truthy in JS), so `confirmPaidOrder` was never invoked for a single real delivery — a live bug in `/api/webhooks/paystack` since Phase 9, undetected through every prior verification pass because every one of those hand-built test payloads simply omitted the `plan` field, making it `undefined` rather than `{}`, which accidentally passed the same broken check every time. Fixed by checking `plan.plan_code` specifically. An earlier hypothesis tried first — that Paystack sent `amount` as a numeric string — turned out to be wrong (the raw dump showed a real number) but the defensive `Number()` coercion was left in place regardless, since it costs nothing and guards against exactly that shape if it's ever true for a different event type. Verified live end-to-end: a real order (from a checkout the widget got stuck on mid-session, ultimately completed on Paystack's side regardless) that had failed to confirm under the old code was manually replayed once under the new code and confirmed correctly, and a genuine Paystack-initiated retry of that same event afterward was handled idempotently with no error. A fully hands-off browser click-through was attempted repeatedly to close the loop with zero manual replay, but Paystack's test checkout widget became unreliable after roughly eight rapid transactions in one session (consistently hanging on "Change Payment Method / Cancel Payment" without progressing) — read as test-mode velocity throttling on Paystack's side, not a regression in this fix. The correctness proof stands on the replay evidence and the retry's idempotency rather than a ninth live click-through |
| 2026-08-16 | **Full audit of this file against the live codebase**, requested by the owner. Found and fixed three stale entries that had drifted from reality: 3.11/3.12 were checked incomplete despite being closed back in Phase 6; the environment readiness table still listed `CRON_SECRET` and `R2_*` as empty despite being set since Phases 9 and 7 respectively. Confirmed live, not just by inspection, that the Resend sending domain the owner set (`NOTIFICATIONS_FROM_EMAIL`) is actually verified and sending — `docs/NOTIFICATIONS.md` updated. Found one real (non-owner) gap: "Export CSV" on Orders and "Import CSV" on Products were dead buttons with no click handler, leftover from the Phase 8b visual redesign, predating the decision that CSV import/export is out of MVP scope (see 8b.13's note). Owner chose to remove them rather than leave dead UI in place; both removed, along with their now-unused `ButtonSecondary` imports. Every other item in this file is either `[x]` verified or a correctly-labeled owner task — see the Open Decisions and per-phase tables for the current, accurate list |
| 2026-08-17 | **Owner-reported bug: order payments now confirm correctly, but subscription payments still don't update the merchant record.** Vercel's Hobby plan only retains 30 minutes of logs, so the owner's original attempt couldn't be inspected after the fact — reproduced live instead with a fresh real subscription payment and the same raw-event-dump technique used for the earlier `charge.success` bug. Root cause: Paystack fires `subscription.create` **exactly once per subscription object**, never again on a later renewal charge against an already-existing subscription — confirmed directly, a second real payment produced only `charge.success`, no `subscription.create` at all. Since `subscriptionStatus` only ever moved to `ACTIVE` from `subscription.create`, every payment after the very first one silently did nothing. Fixed by activating on the plan-linked `charge.success` itself (`handleSubscriptionCharge`) — the signal that genuinely fires on both the first payment and every renewal — with `subscription.create` now only responsible for storing `paystackSubscriptionCode` the first time it's seen. Added `resolveSubscriptionMerchant()`, preferring the exact `metadata.merchantId` already attached at checkout (`startSubscriptionCheckout`) over an email lookup, with email kept as the fallback for renewal charges Paystack initiates on its own outside our checkout flow (which carry no metadata). `activateSubscription()`'s subscription-code parameter is now optional so a renewal charge doesn't clobber the code already stored. Verified live: the merchant's own real payment from this session (which failed to activate under the old code) was reconciled by replaying its exact captured payload under the new code — `subscriptionStatus` flipped to `ACTIVE`, the stored `paystackSubscriptionCode` was preserved rather than cleared, the storefront reactivated, and `/dashboard/settings` correctly shows "Your subscription is active." |
