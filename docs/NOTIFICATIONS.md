...
**Not yet done, deliberately:** the Resend sending domain isn't verified yet (owner task, `NOTIFICATIONS_FROM_EMAIL` is empty in `.env`) — until then, sends fall back to Resend's own unverified-domain address, which only delivers to the Resend account's own signup email. That happened to be the exact address used for verification above, which is why the test sends succeeded. A `RESEND_SETUP.md` (mirroring `R2_SETUP.md`/`VERCEL_SETUP.md`) should be written once domain verification is actually being done.
