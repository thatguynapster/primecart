		...
		aggregate<{ value: number }>("Product", [
			...
			{ $match: { "variants.isActive": true } },
			{
				$group: {
					...
					value: {
						$sum: {
							$multiply: ["$variants.price", "$variants.stock"]
						}
					}
				}
			}
...
/** Paid revenue per calendar day, keyed `YYYY-MM-DD`, over the trailing window. */
	...
	const rows = await aggregate<{ _id: string; total: number }>("Order", [
		{
			$match: {
				merchantId: oid(merchantId),
				paymentStatus: "PAID",
				createdAt: { $gte: isoDate(daysAgo(days - 1)) }
			}
		},
		{
			$group: {
...
export async function getSalesSeries(merchantId: string): Promise<DayBar[]> {
	const byDay = await dailyPaidTotals(merchantId, 14);
	const out: DayBar[] = [];
...
/**
 * Total sales at a chosen granularity (handoff §7 Reporting: "daily / weekly
 * / monthly view"). Daily and weekly both bucket the same trailing window of
 * calendar days — weekly just sums seven of those buckets at a time — so
 * "this week" always means the 7 days ending today, not a calendar week that
 * might be mostly in the future. Monthly reuses `getMonthlyRevenue`'s true
 * calendar-month buckets, which is what a merchant comparing month to month
 * actually wants.
 */
export async function getSalesReport(
	...
	if (granularity === "monthly") return getMonthlyRevenue(merchantId);

	const bucketDays = granularity === "weekly" ? 7 : 1;
	const bucketCount = granularity === "weekly" ? 12 : 14;
	const totalDays = bucketDays * bucketCount;

	const byDay = await dailyPaidTotals(merchantId, totalDays);

	// Daily buckets, oldest first, then chunked into `bucketDays`-wide groups —
	// a single walk covers both granularities.
	const days: number[] = [];
	...
	for (let i = totalDays - 1; i >= 0; i -= 1) {
		...
		// Day-of-month only — mobile-first means up to 14 (daily) or 12 (weekly)
		// columns have to fit a narrow chart; the full date is available on hover
		// via the bar's own title in sales-report.tsx.
		dayLabels.push(date.toLocaleDateString("en-GH", { day: "numeric" }));
	}

	const out: DayBar[] = [];
	for (let i = 0; i < days.length; i += bucketDays) {
		const chunk = days.slice(i, i + bucketDays);
		out.push({
			label: dayLabels[i], // the bucket's first (oldest) day
			value: chunk.reduce((sum, value) => sum + value, 0)
		});
	}
